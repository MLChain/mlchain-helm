...
{{- include "mlchain.db.config" . }}

# The configurations of redis connection.
# It is consistent with the configuration in the 'redis' service below.
{{- include "mlchain.redis.config" . }}

# The configurations of celery broker.
{{- include "mlchain.celery.config" . }}
# Specifies the allowed origins for cross-origin requests to the Web API, e.g. https://mlchain.app or * for all origins.
WEB_API_CORS_ALLOW_ORIGINS: '*'
# Specifies the allowed origins for cross-origin requests to the console API, e.g. https://cloud.mlchain.ai or * for all origins.
...
{{ include "mlchain.storage.config" . }}
{{ include "mlchain.vectordb.config" . }}
{{ include "mlchain.mail.config" . }}
...
CODE_EXECUTION_ENDPOINT: http://{{ template "mlchain.sandbox.fullname" .}}:{{ .Values.sandbox.service.port }}
{{- end }}

{{- if .Values.ssrfProxy.enabled }}
SSRF_PROXY_HTTP_URL: http://{{ template "mlchain.ssrfProxy.fullname" .}}:{{ .Values.ssrfProxy.service.port }}
SSRF_PROXY_HTTPS_URL: http://{{ template "mlchain.ssrfProxy.fullname" .}}:{{ .Values.ssrfProxy.service.port }}
{{- end }}
{{- end }}

{{- define "mlchain.worker.config" -}}
# worker service
...
{{- define "mlchain.vectordb.config" -}}
...
{{- define "mlchain.sandbox.config" -}}
GIN_MODE: release
SANDBOX_PORT: '8194'
{{- if .Values.ssrfProxy.enabled }}
HTTP_PROXY: http://{{ template "mlchain.ssrfProxy.fullname" .}}:{{ .Values.ssrfProxy.service.port }}
HTTPS_PROXY: http://{{ template "mlchain.ssrfProxy.fullname" .}}:{{ .Values.ssrfProxy.service.port }}
{{- end }}
{{- end }}

{{- define "mlchain.nginx.config.proxy" }}
...
{{- define "mlchain.nginx.config.nginx" }}
user  nginx;
...
{{- define "mlchain.nginx.config.default" }}
server {
    listen 80;
    server_name _;

    location /console/api {
      proxy_pass http://{{ template "mlchain.api.fullname" .}}:{{ .Values.api.service.port }};
      include proxy.conf;
    }
    ...
    location / {
      proxy_pass http://{{ template "mlchain.web.fullname" .}}:{{ .Values.web.service.port }};
      include proxy.conf;
    }
}
{{- end }}

{{- define "mlchain.ssrfProxy.config.squid" }}
...
cache_peer {{ template "mlchain.sandbox.fullname" .}} parent {{ .Values.sandbox.service.port }} 0 no-query originserver
acl src_all src all
